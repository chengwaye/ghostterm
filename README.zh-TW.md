...
3. **C++ 編譯工具**（`node-pty` 需要編譯原生模組）
   - Windows：安裝 [Visual Studio Build Tools](https://visualstudio.microsoft.com/visual-cpp-build-tools/)，勾選「使用 C++ 的桌面開發」
   - macOS：`xcode-select --install`
   - Linux：`sudo apt install build-essential`（Debian/Ubuntu）
4. **Claude Code** 裝在電腦（`npm install -g @anthropic-ai/claude-code`）
